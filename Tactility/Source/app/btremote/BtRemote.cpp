...
Profiles defaults() { return {{
 ...
 {"Browser", {{b("Tab nou","plus",key(0x17,1)),b("Redeschide","undo",key(0x17,3)),b("Inchide tab","close",key(0x1A,1)),b("Tab stanga","prev",key(0x2B,3)),b("Tab dreapta","next",key(0x2B,1)),b("Refresh","refresh",key(0x15,1)),b("Adresa","search",key(0x0F,1)),b("Back","back",key(0x50,4)),b("Forward","next",key(0x4F,4)),b("Sus","up",key(0x4B)),b("Jos","down",key(0x4E)),b("Full screen","screen",key(0x44))}}}
}}; }
...
struct Context { uint32_t appInstanceId=0; Device*hid=nullptr; lv_obj_t*status=nullptr,*profileLabel=nullptr,*grid=nullptr; Profiles profiles=defaults(); size_t currentProfile=0; std::unique_ptr<tt::Timer> statusTimer; };
void updateStatus(Context*c){if(!c->status)return;lv_label_set_text(c->status,c->hid&&bluetooth_hid_device_is_connected(c->hid)?"Conectat":"Cauta Tactility Remote");}
...
void createWidgets(lv_obj_t*p,void*d){auto*c=static_cast<Context*>(d);lv_obj_set_flex_flow(p,LV_FLEX_FLOW_COLUMN);lv_obj_set_style_pad_row(p,0,0);auto*t=lvgl_toolbar_create(p,"Bluetooth Remote v2");lvgl_toolbar_set_nav_action(t,LV_SYMBOL_CLOSE,onBack,c);auto*co=lv_obj_create(p);lv_obj_set_width(co,LV_PCT(100));lv_obj_set_flex_grow(co,1);lv_obj_set_style_border_width(co,0,0);lv_obj_set_style_pad_all(co,6,0);lv_obj_set_style_bg_color(co,lv_color_hex(0x181A1D),0);lv_obj_set_style_text_color(co,lv_color_white(),0);lv_obj_set_flex_flow(co,LV_FLEX_FLOW_COLUMN);auto*n=lv_obj_create(co);lv_obj_set_width(n,LV_PCT(100));lv_obj_set_height(n,30);lv_obj_set_style_border_width(n,0,0);lv_obj_set_style_pad_all(n,0,0);lv_obj_set_flex_flow(n,LV_FLEX_FLOW_ROW);for(size_t i=0;i<PROFILE_COUNT;i++){auto*bt=lv_button_create(n);lv_obj_set_flex_grow(bt,1);lv_obj_set_height(bt,28);lv_obj_set_user_data(bt,reinterpret_cast<void*>(i));auto*l=lv_label_create(bt);lv_label_set_text(l,c->profiles[i].name.c_str());lv_obj_center(l);lv_obj_add_event_cb(bt,onProfile,LV_EVENT_SHORT_CLICKED,c);}c->profileLabel=lv_label_create(co);lv_obj_set_style_text_font(c->profileLabel,lvgl_get_text_font(FONT_SIZE_SMALL),0);c->status=lv_label_create(co);updateStatus(c);c->grid=lv_obj_create(co);lv_obj_set_width(c->grid,LV_PCT(100));lv_obj_set_flex_grow(c->grid,1);lv_obj_set_style_border_width(c->grid,0,0);lv_obj_set_style_pad_all(c->grid,2,0);lv_obj_set_flex_flow(c->grid,LV_FLEX_FLOW_ROW_WRAP);lv_obj_set_flex_align(c->grid,LV_FLEX_ALIGN_SPACE_EVENLY,LV_FLEX_ALIGN_SPACE_EVENLY,LV_FLEX_ALIGN_CENTER);drawGrid(c);}
int32_t appMain(int,char**){auto*c=new Context();c->appInstanceId=app_scheduler_current_app_id();c->profiles=loadProfiles();c->hid=bluetooth_hid_device_get();
 // Only (re)start advertising when not already connected: reopening this app while a peer is
 // still connected (e.g. app was closed and reopened without disconnecting) would otherwise
 // discard the live BleHidDeviceCtx/conn_handle and desync the app from the real connection state.
 if(c->hid&&!bluetooth_hid_device_is_connected(c->hid))bluetooth_hid_device_start(c->hid,BT_HID_DEVICE_MODE_KEYBOARD);
 TaskEventGroup eg{};task_event_group_construct(&eg);AppEventSubscription s{};check(app_event_subscribe(&s,&eg)==ERROR_NONE);WindowId w=window_manager_create(c->appInstanceId,createWidgets,c);
 c->statusTimer=std::make_unique<tt::Timer>(tt::Timer::Type::Periodic,pdMS_TO_TICKS(1000),[c,w]{if(!lvgl_is_running())return;lvgl_lock();if(window_manager_get_state(w)==WINDOW_STATE_GRANTED)updateStatus(c);lvgl_unlock();});
 c->statusTimer->start();
 bool close=false;while(!close){task_event_group_wait_any(&eg,nullptr,portMAX_DELAY);AppEvent e{};while(app_event_poll(&s,&e)==ERROR_NONE)if(e.type==APP_EVENT_CLOSE)close=true;}
 c->statusTimer->stop();window_manager_remove(w);if(c->hid)device_put(c->hid);check(app_event_unsubscribe(&s)==ERROR_NONE);task_event_group_destruct(&eg);delete c;return 0;}
}
